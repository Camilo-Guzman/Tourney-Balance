...
- The build scripts depend on renaming the repository folder to `TourneyBalance`.
- Don't rename the cloned repository.
...
#### Building the Mod
Don't use `_Build Mod.bat` to build this mod.
...
- **Important**:
    ...
    - Otherwise `[ERROR] Access Denied or folder not found.`

#### Uploading the Mod
We don't need the `_Upload Mod.bat` script to update the mod in the steam workshop, instead we use GitHub workflows for handling the upload. This way we can track changes to the mod via the commit history.
