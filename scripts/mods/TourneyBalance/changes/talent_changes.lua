...
mod:add_proc_function("gain_markus_mercenary_passive_proc", function (owner_unit, buff, params)
	...
	if ALIVE[owner_unit] and target_number and 3 <= target_number and (attack_type == "light_attack" or attack_type == "heavy_attack") then
		local side = Managers.state.side.side_by_unit[owner_unit] -- The above line makes sure that Enhanced Training only requires 3 enemies hit.
		local player_and_bot_units = side.PLAYER_AND_BOT_UNITS -- A talent buff template needs changing too but that should be around here somewhere.
		...
		elseif talent_extension:has_talent("markus_mercenary_passive_power_level_on_proc", "empire_soldier", true) then  -- Reikland Reaper
			buff_system:add_buff(owner_unit, "markus_mercenary_passive_power_level", owner_unit, false) -- I've thrown Reikland Reaper in here too to keep everything in one place.
		end	
	...
	if ALIVE[owner_unit] and target_number and 1 <= target_number and (attack_type == "light_attack" or attack_type == "heavy_attack") then
		if talent_extension:has_talent("markus_mercenary_passive_group_proc", "empire_soldier", true) then
			local side = Managers.state.side.side_by_unit[owner_unit]
			local player_and_bot_units = side.PLAYER_AND_BOT_UNITS
			local num_units = #player_and_bot_units

			for i = 1, num_units do  						-- This is Strike Together being added on as a passive.
				local unit = player_and_bot_units[i] 			-- Setting target_number to 1 above means that this will procc after hitting 1 enemy.
